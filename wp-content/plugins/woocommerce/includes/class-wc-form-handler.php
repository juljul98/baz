			...
			try {
				...
				if ( is_email( $username ) && apply_filters( 'woocommerce_get_username_from_email', true ) ) {
					...
					if ( isset( $user->user_login ) ) {
						...
						throw new Exception( '<strong>' . __( 'Error', 'woocommerce' ) . ':</strong> ' . __( "The email and password you entered don't match. ".'<a href="'. wp_lostpassword_url().'">'.__( 'Need Help?' ) .'</a>', 'woocommerce' ) );
					}
